...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById("searchWindow").addEventListener("keypress", openWindowResulr);
    document.getElementById("resistorSearch").addEventListener("click", openWindowResulrResistors);
    document.getElementById("length_power").addEventListener("keypress", checkKeyPress)
});
...
//Функція що відкриває нову вкладку з результатом пошуку по натисканню клавіші "Enter"
function openWindowResulr(event) {
    if (event.keyCode === 13) {
        let result = document.getElementById("searchWindow").value;
        let url = "https://www.rcscomponents.kiev.ua/modules.php?name=Asers_Shop&s_op=search&query=";
        window.open(url + result)
    }
}

//Функція що відкриває нову вкладку з результатом пошуку резистора по вказаних параметрах у формі
function openWindowResulrResistors() {
    let result = document.getElementById("input_resistor_resistance").value;
    let url = "https://www.rcscomponents.kiev.ua/modules.php?name=Asers_Shop&s_op=search&query=";
    window.open(url + result)
}
