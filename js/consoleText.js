...
function stock() {
  ...
  var kilkist_ryadkiv_svitli = document.getElementsByClassName(
	"productlist-table-row-odd"
  ).length;
  var kilkist_ryadkiv_temni = document.getElementsByClassName(
  ...
  for (var a = 0; a < kilkist_ryadkiv_svitli; a++) {
	var nayavnist_ryadok_svitli = document
	  .getElementsByClassName("productlist-table-row-odd")
	  .item(a);
	//        console.log('Світлий рядок №' + a + ' =');
	//        console.log(nayavnist_ryadok_svitli);
	var nayavnist_ryadok_dovguna_svitli = nayavnist_ryadok_svitli.getElementsByTagName(
	  "td"
	).length;
	...
	if (nayavnist_ryadok_dovguna_svitli > 7) {
	  ...
	  var nayavnist_komirka_svitli = nayavnist_ryadok_svitli
		.getElementsByTagName("td")
		.item(nayavnist_ryadok_dovguna_svitli - 2);
  ...
  for (var b = 0; b < kilkist_ryadkiv_temni; b++) {
	var nayavnist_ryadok_temni = document
	  .getElementsByClassName("productlist-table-row-even")
	  .item(b);
	//        console.log('Темний рядок №' + b + ' =');
	//        console.log(nayavnist_ryadok_temni);
	var nayavnist_ryadok_dovguna_temni = nayavnist_ryadok_temni.getElementsByTagName(
	  "td"
	).length;
	//        console.log('Кількість колонок(елементів) в рядку = ' + nayavnist_ryadok_dovguna_temni);
	if (
	  nayavnist_ryadok_dovguna_temni > 7 &&
	  nayavnist_ryadok_dovguna_temni <= 10
	) {
	  var newCol1 = nayavnist_ryadok_temni.getElementsByTagName("td").item(2);
	  newCol1.style.color = "red";
	}
	if (
	  nayavnist_ryadok_dovguna_temni >= 11 ||
	  nayavnist_ryadok_dovguna_temni == 9
	) {
	  var newCol1 = nayavnist_ryadok_temni.getElementsByTagName("td").item(5);
	  newCol1.style.color = "red";
	} else {
	  var nayavnist_komirka_temni = nayavnist_ryadok_temni
		.getElementsByTagName("td")
		.item(nayavnist_ryadok_dovguna_temni - 2);
	  //                console.log('Колонка в якій треба змінити колір тексту =');
...
function getTarget(event) {
  ...
  if (
	searchRow.search("код товара") >= 0 ||
	searchRow.search("код товару") >= 0
  ) {
	navigator.clipboard
	  ...
	  .then(() => {
		...
		attBox.value =
		  "background-color: #1a1a1a; color: #f2f2f2; height: 18px; border-radius: 4px 4px 4px 0; text-align: center; padding: 2px 10px; position: absolute;" +
		  "top: " +
		  yBox +
		  "px; left: " +
		  xBox +
		...
		attTriangle.value =
		  "border-width: 5px; border-style: solid; border-color: #1a1a1a transparent transparent #1a1a1a; position: absolute;" +
		  "top: " +
		  (y - 12) +
		  "px; left: " +
		  (x + 10) +
...
/**
 * @description Make filters in group of goods higher.
 */
function filterHigher(f) {
	// const f = document.querySelectorAll('.catalog_filter');
	f.forEach((i) => (i.style.height = "200px"));
}

let status = true;

function run() {
	const attempts = [5, 10, 15, 20, 25, 30];
